...
SYSTEM_PROMPT: str = (
    "常に日本語で考えてください。\n\n"
    "以下の数学問題をステップバイステップで解いてください。"
)
    # "各ステップの推論は<think>ここに推論プロセス</think>の形式で記述してください。\n"
    # "最後に、最終的な答えを \\boxed{} で囲んで提供してください。\n\n"
    # "問題：{text}\n\n"
    # "答えは「答えは：\\boxed{final_answer}」の形式で<think></think>タグ内に推論、\\boxed{}内に答えを記述してください。"
...
def language_reward(completions: List[Any], **kwargs) -> List[float]:
    """Reward function that checks if the completion is in Japanese.

    Args:
        completions (List[Any]): List of completions, each a list of dicts with a "content" key.

    Returns:
        List[float]: 1.0 if the completion is in Japanese, else 0.0.
    """
    rewards: List[float] = []
    for completion in completions:
        if (
        ...
        try:
            ...
            rewards.append(lang["ja"]*0.2 if "ja" in lang else 0.0)
        except Exception as e:
...
def main() -> None:
    ...
    # model_id = "shanchen/ds-limo-ja-full"
    ...
    # Load datasets
    # train_dataset_name: str = "shanchen/limo_ja"
    train_dataset_name: str = "jacobmorrison/MATH-500-japanese"
    if "limo" in train_dataset_name:
        train_dataset: Dataset = load_dataset("shanchen/limo_ja", split="train")
        train_dataset = train_dataset.map(make_conversation)
        logger.info(f"using limo dataset as train dataset")
    else:
        train_dataset: Dataset = load_dataset("jacobmorrison/MATH-500-japanese", split="test")
        logger.info(f"using math500 dataset as train dataset")
    ...
    test_dataset: Dataset = load_dataset(
        "shanchen/aime_2025_multilingual", "default", split="ja"
    )
    ...
    training_args = GRPOConfig(
        output_dir=f"/temp_work/ch225816/trl_grpo/{id}_0.28_0.2f_0.2lang_64_{train_dataset_name}_8lora_1e-6",
        learning_rate=1e-6,
