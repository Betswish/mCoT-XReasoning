...
SYSTEM_PROMPT: str = (
    "Siempre piensa en español.\n\n"
    "Resuelve el siguiente problema de matemáticas paso a paso."
)
    # "Describe cada paso de razonamiento en la forma <think>proceso de razonamiento aquí</think>.\n"
    # "Finalmente, proporciona tu respuesta final rodeándola con \\boxed{}.\n\n"
    # "Problema: {text}\n\n"
    # "La respuesta debe estar en el formato « La respuesta es: \\boxed{respuesta_final} » con el razonamiento en las etiquetas <think></think> y la respuesta en \\boxed{}."
...
def format_reward(completions: List[Any], **kwargs) -> List[float]:
    """Reward function that checks if the completion matches the required Spanish math prompt format.
    ...
    Example:
        >>> completions = [[{"content": "<think>razonamiento</think>\nLa respuesta es: \\boxed{42}"}]]
...
def language_reward(completions: List[Any], **kwargs) -> List[float]:
    """Reward function that checks if the completion is in Spanish.

    Args:
        completions (List[Any]): List of completions, each a list of dicts with a "content" key.

    Returns:
        List[float]: 1.0 if the completion is in Spanish, else 0.0.
    """
    rewards: List[float] = []
    for completion in completions:
        if (
        ...
        try:
            ...
            rewards.append(lang["es"]*0.2 if "es" in lang else 0.0)
        except Exception as e:
...
def main() -> None:
    ...
    # model_id = "shanchen/ds-limo-ja-full"  # You may want to use a Spanish-compatible model
    ...
    # Load datasets - using Spanish MATH dataset
    train_dataset_name: str = "bezir/MATH-500-multilingual"
    
    # Load Spanish subset of MATH-500-multilingual
    train_dataset: Dataset = load_dataset("bezir/MATH-500-multilingual", "Spanish", split="test")
    logger.info(f"using Spanish MATH-500 dataset as train dataset")
    ...
    # For test dataset, you might want to use a different Spanish math dataset
    # or create a validation split from the same dataset
    test_dataset: Dataset = load_dataset(
        "shanchen/aime_2025_multilingual", "default", split="es"
    )
    ...
    training_args = GRPOConfig(
        output_dir=f"/temp_work/ch225816/trl_grpo/nokl_{id}_es_dapo_format_lang_0.28_0.2f_0.2lang_64_{train_dataset_name.replace('/', '_')}_es_8lora_7e-6",
        learning_rate=7e-6,
